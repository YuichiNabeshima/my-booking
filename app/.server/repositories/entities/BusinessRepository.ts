  ...
  implements IBusinessRepository<BusinessRepositoryDTO>
{
  constructor(
    ...
    transactionManager: ITransactionManager<
      Record<PascalToCamelCase<Prisma.ModelName>, Prisma.BusinessDelegate>
    >,
  ) {
    super({
      modelName: 'business',
    ...
    const where = {
      AND: [
        ...
        args.cuisine?.filter(Boolean).length
          ...
          : {},
        args.neighborhood?.filter(Boolean).length
          ? { neighborhood: { in: args.neighborhood.filter(Boolean) } }
          : {},
    ...
    const where = {
      AND: [
        ...
        params.cuisine?.filter(Boolean).length
          ...
          : {},
        params.neighborhood?.filter(Boolean).length
          ? { neighborhood: { in: params.neighborhood.filter(Boolean) } }
          : {},
