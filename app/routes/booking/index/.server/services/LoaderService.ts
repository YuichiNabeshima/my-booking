...
export class LoaderService implements ILoaderService {
  ...
  async execute({
    ...
    price,
    neighborhood,
  }: LoaderServiceArgsDTO): Promise<LoaderServiceResultDTO> {
    ...
    const businesses = await this.businessRepository.fetchBusinesses({
      take,
      skip,
      cuisine: cuisines,
    ...
    const totalCount = await this.businessRepository.getTotalCount({
      cuisine: cuisines,
      price_level: priceLevel,
      neighborhood: neighborhoods,
    });
