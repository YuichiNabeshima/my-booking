...
export class ActionService implements IActionService {
  constructor(
    ...
    @inject(GLOBAL_DI_TYPES.BusinessTagRepository) private businessTagRepository: IBusinessTagRepository,
  ) {}
  ...
  async execute({ cookie, tags: newTags }: ActionServiceArgsDTO): Promise<ActionServiceResultDTO> {
    ...
    const toUpdate = newTagsMapedToDb.filter(item =>
    ...
    const toDelete = tags.filter(tag =>
      !newTagsMapedToDb.some(newtag => newtag.id === tag.id)
    );
