  ...
  useEffect(() => {
    if (result) {
      const status = result.status === STATUS.SUCCESS ? 'success'
                      ...
                      : 'error';
      const message = result.status === STATUS.SUCCESS ? 'updated!'
        ...
        <CardContent className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(value) => form.update({ [fields[FORM_NAME.CUISINE_KIND].name]: value })}
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => form.update({ [fields[FORM_NAME.PRICE_LEVEL].name]: value })}
            ...
            >
              ...
              <SelectContent>
                {(Object.keys(PRICE_LABEL) as unknown as PriceLevel[]).map((price) => (
                  <SelectItem key={price} value={String(price)}>{PRICE_DISPLAY_LABEL[price]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="mt-4 flex justify-end w-full">
            <Button size="lg" className="w-full cursor-pointer">Save</Button>
          </div>
        </CardContent>
      </Form>
    </Card>
